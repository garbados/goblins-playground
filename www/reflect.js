...
class Pair extends HeapObject {
  ...
  repr () {
    const car_repr = repr(this.reflector.car(this))
    const cdr_repr = repr(this.reflector.cdr(this))
...
class Vector extends HeapObject {
  ...
  repr () {
    const len = this.reflector.vector_length(this)
    let out = '#('
    for (let i = 0; i < len; i++) {
      if (i) out += ' '
...
class Bytevector extends HeapObject {
  ...
  repr () {
    const len = this.reflector.bytevector_length(this)
    let out = '#vu8('
    for (let i = 0; i < len; i++) {
      if (i) out += ' '
...
class Bitvector extends HeapObject {
  ...
  repr () {
    const len = this.reflector.bitvector_length(this)
    let out = '#*'
    for (let i = 0; i < len; i++) {
...
function instantiate_streaming (path, imports) {
  if (typeof fetch !== 'undefined' && typeof window !== 'undefined') { return WebAssembly.instantiateStreaming(fetch(path), imports) }
  let bytes
...
class Scheme {
  ...
  static async reflect (abi, { reflect_wasm_dir = '.' }) {
    ...
    const reflect_wasm = reflect_wasm_dir + '/reflect.wasm'
    const rt = {
  ...
  #init_module (mod) {
    ...
    const proc = new Procedure(this, mod.get_export('$load').value)
    return proc.call()
  }

  static async load_main (path, opts = {}) {
    const mod = await SchemeModule.fetch_and_instantiate(path, opts)
    const reflect = await mod.reflect(opts)
  ...
  #to_scm (js) {
    ...
    if (typeof (js) === 'number') {
      ...
      if (BigInt(api.scm_most_negative_fixnum()) <= js &&
                js <= BigInt(api.scm_most_positive_fixnum())) { return api.scm_from_fixnum(Number(js)) }
  ...
  call (func, ...args) {
    ...
    for (const [idx, arg] of args.entries()) { api.vector_set(argv, idx + 1, this.#to_scm(arg)) }
    argv = api.call(func, argv)
    const results = []
    for (let idx = 0; idx < api.vector_length(argv); idx++) { results.push(this.#to_js(api.vector_ref(argv, idx))) }
    return results
  }
...
const async_invoke = typeof queueMicrotask !== 'undefined'
  ? queueMicrotask
...
function wtf8_to_string (wtf8) {
  const { as_iter, iter_next } = wtf8_helper.exports
  const codepoints = []
  const iter = as_iter(wtf8)
...
function string_to_wtf8 (str) {
  const { string_builder, builder_push_codepoint, finish_builder } =
        wtf8_helper.exports
  const builder = string_builder()
  for (const cp of str) { builder_push_codepoint(builder, cp.codePointAt(0)) }
  return finish_builder(builder)
}

async function load_wtf8_helper_module (reflect_wasm_dir = '') {
  if (wtf8_helper) return
  const wtf8_wasm = reflect_wasm_dir + '/wtf8.wasm'
  const { module, instance } = await instantiate_streaming(wtf8_wasm)
...
class SchemeModule {
  ...
  static #rt = {
    ...
    bignum_is_i64 (n) {
      return n >= -0x8000_0000_0000_0000n && n <= 0x7FFF_FFFF_FFFF_FFFFn
    },
    bignum_is_u64 (n) {
  ...
  static #code_origin (code) {
    ...
    for (const mod of SchemeModule.#all_modules) {
  ...
  static #code_name (code) {
    const [mod, idx] = SchemeModule.#code_origin(code)
  ...
  constructor (instance) {
    ...
    this.#instance = instance
    const open_file_error = (filename) => {
    ...
    if (typeof printErr === 'function') { // v8/sm dev console
      ...
      this.#io_handler = {
        ...
        read_stdin: () => {
          const n = fs.readSync(process.stdin.fd, stdinBuf, 0, stdinBuf.length)
  ...
  static async fetch_and_instantiate (path, {
    abi, reflect_wasm_dir = '.',
    user_imports = {}
  }) {
    ...
    const { module, instance } = await instantiate_streaming(path, imports)
    const mod = new SchemeModule(instance)
    return mod
  }
  ...
  exported_abi () {
    const abi = {}
    for (const [k, v] of Object.entries(this.all_exports())) {
